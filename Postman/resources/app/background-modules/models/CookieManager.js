...
class CookieManager {
   ...
   */
  getCookies (callback) {
    const session = require('electron').session.fromPartition('persist:postman_user_cookies');
   ...
   * @returns {undefined}
   */
  pushCookies (cookies, url, finalCallback) {
    ...
    async.series([
      ...
      (callback) => {
        this.getCookies((getCookiesErr, existingCookies) => {
          if (getCookiesErr) {
