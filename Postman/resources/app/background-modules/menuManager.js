var electron = require('electron'),
    ...
    dockMenuTemplate = [
      { label: 'New Collection', click: function(menuItem, browserWindow, options) { menuManager.handleMenuAction('newCollection', null, options) } },
      { label: 'New Window ',  click: function(menuItem, browserWindow, options) { menuManager.handleMenuAction('newWindow', null, options) } }
    ];
menuManager = {
  ...
  handleMenuAction: function(action, meta, options) {
    if(action === "reloadWindow") {
      ...
      var win = BrowserWindow.getFocusedWindow();
      win.webContents.send('electronWindowMessage', {
        name: "internalEvent",
        data: {
          event: action
        }
